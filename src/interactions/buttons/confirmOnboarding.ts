    ...
    async execute(interaction) {
        ...
        if (!std) {
            await interaction.update({
                content: `<@${interaction.user.id}> ไม่พบข้อมูลของคุณ กรุณาติดต่อฝ่ายประธาน`,
                embeds: [],
                components: [],
            });
            return;
        ...
        try {
            if (!std.role) {
                await interaction.update({
                    content: `<@${interaction.user.id}> ไม่พบ role ของ staff นี้ กรุณาติดต่อฝ่ายประธาน`,
                    embeds: [],
                    components: [],
                });
                return;
            ...
            await db.update(staffs).set({ userId: interaction.user.id }).where(eq(staffs.studentId, studentId));
