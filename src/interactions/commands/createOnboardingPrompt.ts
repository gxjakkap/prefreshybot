...
const createOnboardingPrompt: Command = {
    ...
    async execute(interaction) {
        ...
        const caption = (interaction.options.getString("message") ?? "พิมพ์ `/link` เพื่อลงทะเบียนและรับยศตามฝ่าย").replace(
            /\\n/g,
            "\n",
        );
        ...
        const channel = channelOption ? interaction.client.channels.cache.get(channelOption.id) : interaction.channel;
        ...
        if (messageType === "embed") {
            const embed = new EmbedBuilder().setDescription(caption).setColor(0x5865f2);
